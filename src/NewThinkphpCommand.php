...
namespace Php\Installer\Console;
...
class NewThinkphpCommand extends Command
{
    /**
    ...
    protected function configure()
    {
        $this
            ->setName('tp51')
            ->setDescription('Create a new thinkphp5.1 application')
            ->addArgument('name', InputArgument::REQUIRED)
            ->addOption('dev', null, InputOption::VALUE_NONE, 'Installs the latest thinkphp5.1 release')
...
`---'`---'`---'`---'`---'`    `---'`---^` ' '</>".PHP_EOL.PHP_EOL);
            ...
            _____  _     _  _      _  __ ____  _     ____ 
            /__ __\/ \ /|/ \/ \  /|/ |/ //  __\/ \ /|/  __\
              / \  | |_||| || |\ |||   / |  \/|| |_|||  \/|
              | |  | | ||| || | \|||   \ |  __/| | |||  __/
              \_/  \_/ \|\_/\_/  \|\_|\_\\_/   \_/ \|\_/   (</>'.PHP_EOL.PHP_EOL);
        }
        ...
        $commands = [
            $composer." create-project orchief/think51 \"$directory\" $version --remove-vcs --prefer-dist",
        ];
    ...
    protected function createRepository(string $directory, InputInterface $input, OutputInterface $output)
    {
        ...
        $commands = [
            'git init -q -b main .',
            'git add .',
            'git commit -q -m "Set up a fresh Thinkphp51 app"',
        ];
